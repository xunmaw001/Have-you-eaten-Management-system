<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="店铺类别" prop="dianpuleibie">
          <el-input v-model="ruleForm.dianpuleibie" 
              placeholder="店铺类别" clearable  :readonly="ro.dianpuleibie"></el-input>
        </el-form-item>
        <div v-else>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `dianpufenlei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.dianpufenleiCrossAddOrUpdateFlag = false;
                  this.parent.search();
